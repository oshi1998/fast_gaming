...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    od_payment_method,od_status) VALUES (:id,:cus,:amount,:pro,:shipping,:discount,:total,:pm,:status)";
    ...
    if ($result) {
        if (isset($_SESSION['CART_DISCOUNT_CODE']) && !empty($_SESSION['CART_DISCOUNT_CODE'])) {
            ...
            if ($result) {
                ...
                if ($query == count($_SESSION['MYCART'])) {
                ...
                http_response_code(412);
                echo json_encode(['message' => "อัพเดตข้อมูลการใช้ส่วนลดไม่สำเร็จ"]);
                exit;
            }
        }
    } else {
